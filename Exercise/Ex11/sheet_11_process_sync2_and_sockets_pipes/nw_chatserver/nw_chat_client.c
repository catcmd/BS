...
int main(int argc, char** argv) {
    ...
    //Hint:
    inet_aton("141.60.33.159", &address.sin_addr); // - use "inet_aton(...)" to convert internet host address to binary form
    address.sin_port        = htons(PORT);// - use "htons(...)" convert values between host and network byte order

    //TODO: Connect to server
    int connection_result = connect(network_socket, (struct sockaddr*) &address, (sizeof address));
    if(connection_result != 0) {
        printf("Error: can't connect to that god dang address\n");
        exit(EXIT_FAILURE);
    }
